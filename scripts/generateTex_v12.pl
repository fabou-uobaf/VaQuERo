...
while(<I>){
  ...
  if(scalar(@F) <= 1){
    ...
    $data_m{$F[0]}->{$F[1]}->{$F[2]} = $F[3];
  }
  elsif(scalar(@F) == 5){
    $data_m{$F[0]}->{$F[1]}->{$F[2]}->{$F[3]} = $F[4];
  }
  elsif(scalar(@F) == 6){
    $data_m{$F[0]}->{$F[1]}->{$F[2]}->{$F[3]}->{$F[4]} = $F[5];
  }
  elsif(scalar(@F) == 7){
    $data_m{$F[0]}->{$F[1]}->{$F[2]}->{$F[3]}->{$F[4]}->{$F[5]} = $F[6];
  }
  else{
    print STDERR "Warning: more than 7 entries in line $.\n";
  }
}
close I;
...
if (defined($data_m{growing_excessmutations}->{overview}->{plot}) && defined($data_m{growing_excessmutations}->{overview}->{map})) {
    ...
    my $path_map = $data_m{growing_excessmutations}->{overview}->{map}->{filtered};
    my $path_table = $data_m{growing_excessmutations}->{overview}->{table}->{filtered};
    &printMutationOverview($path_plot, $path_map, $path_table);
}
else{
