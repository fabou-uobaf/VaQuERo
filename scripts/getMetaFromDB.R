  ...
  left_join(x = samples_with_seqdata_db, y = sewagePlants_db, by = "LocationID_coronA", suffix = c(".db",".location")) -> samples_with_seqdata_with_wwplants_db
  ...
  samples_with_seqdata_with_wwplants_db %>% mutate(adress_town = ifelse(is.na(adress_town) & !is.na(LocationName_coronA), LocationName_coronA, adress_town)) -> samples_with_seqdata_with_wwplants_db
  ...
  samples_with_seqdata_with_wwplants_db %>% mutate(connected_people_2018 = ifelse(is.na(connected_people_2018), 1234, connected_people_2018)) -> samples_with_seqdata_with_wwplants_db
  ...
  samples_with_seqdata_with_wwplants_db %>% filter(! is.na(BSF_run) & ! is.na(BSF_sample_name) & ! is.na(BSF_start_date) & ! is.na(LocationID_coronA) & ! is.na(LocationName_coronA) & ! is.na(N_in_Consensus) & ! is.na(RNA_ID_int) & ! is.na(adress_town) & ! is.na(connected_people_2018) & ! is.na(dcpLatitude) & ! is.na(dcpLongitude) & ! is.na(include_in_report) & ! is.na(report_category) & ! is.na(sample_date) & ! is.na(status)) -> samples_with_seqdata_with_wwplants_db
  
  coi <- c("BSF_run", "BSF_sample_name", "BSF_start_date", "LocationID_coronA", "LocationName_coronA", "N_in_Consensus", "RNA_ID_int", "additional_information", "adress_town", "connected_people_2018", "dcpLatitude", "dcpLongitude", "include_in_report", "report_category", "sample_date", "status")
