...
for (r in 1:length(unique(sewage_samps.dt$LocationID))) {
    ...
    if(opt$verbose | !opt$verbose){
        ...
        if(dim(outbreak.dt)[1]>0){
            ...
            plot.width <- 8+length(unique(outbreak.dt$ID))/5
            plot.height <- 3+length(labelsToUse)/3
